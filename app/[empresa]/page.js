...
export default function PropostaPage({ params }) {
  ...
  useEffect(() => {
    // Resolver params se for Promise
    Promise.resolve(params).then((resolvedParams) => {
      const emp = resolvedParams.empresa
      setEmpresa(emp)
      const prop = propostas[emp]
      if (!prop) {
        notFound()
      }
      setProposta(prop)
  ...
  if (!proposta) {
    return <div style={{ background: '#0E0E0E', color: '#FFF', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Carregando...</div>
  }

  const { nome, tagline, descricao, whatsapp, planos, decisoes } = proposta
